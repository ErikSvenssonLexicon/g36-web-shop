package se.lexicon.g36webshop.model.misc;
    ...
    public static OrderStatus of(String name){
        if(name != null){
            for(OrderStatus orderStatus : OrderStatus.values()){
                if(orderStatus.getName().equalsIgnoreCase(name)){
                    return orderStatus;
                }
            }
        }
        throw new IllegalArgumentException("No enum of type OrderStatus matches " + name);
    }

    public static OrderStatus of(int orderStatusNumber){
        for(OrderStatus orderStatus : OrderStatus.values()){
            if(orderStatus.getOrderStatusNumber() == orderStatusNumber){
                return orderStatus;
            }
        }
        throw new IllegalArgumentException("No enum of type OrderStatus matches " + orderStatusNumber);
    }
}
