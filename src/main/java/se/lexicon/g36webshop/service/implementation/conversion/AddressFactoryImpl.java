...
public class AddressFactoryImpl implements AddressFactory {
    ...
    public Address createFromDTO(AddressDTO addressDTO) {
        ...
        address.setStreet(addressDTO.getStreet() == null ? null : addressDTO.getStreet().trim());
        if(addressDTO.getZipCode() != null){
            address.setZipCode(addressDTO.getZipCode().trim().replaceAll("\\p{Punct}", "").replaceAll("\\p{Space}", ""));
        }
        address.setCity(addressDTO.getCity() == null ? null : addressDTO.getCity().trim());
        address.setCountry(addressDTO.getCountry() == null ? null : addressDTO.getCountry().trim());
        return address;
    }
}
